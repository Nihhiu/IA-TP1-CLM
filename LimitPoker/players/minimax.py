...
class LimitPokerMinimaxBot(LimitPokerPlayer):
    ...
    def __heuristic(self, state: LimitPokerState):
        grid = state.get_grid()
        longest = 0
    ...
    def minimax(self, state: LimitPokerState, depth: int, alpha: int = -math.inf, beta: int = math.inf,
                is_initial_node: bool = True):
        # first we check if we are in a terminal node (victory, draw or loose)
        if state.is_finished():
            return {
                LimitPokerResult.WIN: 40,
                LimitPokerResult.LOOSE: -40,
                LimitPokerResult.DRAW: 0
