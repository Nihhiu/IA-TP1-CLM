...
class MinesweeperMinimaxBot(MinesweeperPlayer):
    ...
    def __heuristic(self, state: MinesweeperState):
        grid = state.get_grid()
        longest = 0
    ...
    def minimax(self, state: MinesweeperState, depth: int, alpha: int = -math.inf, beta: int = math.inf,
                is_initial_node: bool = True):
        # first we check if we are in a terminal node (victory, draw or loose)
        if state.is_finished():
            return {
                MinesweeperResult.WIN: 40,
                MinesweeperResult.LOOSE: -40,
                MinesweeperResult.DRAW: 0
